...
export default function MovieDetailPage() {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      // For OMDB movies, we need to create or find the movie in local DB first
      let finalMovieId: string | null = null;
      const isLocal = movieId.startsWith('local-');
      const actualId = isLocal ? movieId.replace('local-', '') : movieId.replace('omdb-', '');

      if (isLocal) {
        finalMovieId = actualId;
      } else {
        // For OMDB movies, create local entry if it doesn't exist
        ...
        if (createResponse.ok) {
          ...
          const findResponse = await fetch(`/api/movies/find?title=${encodeURIComponent(movie?.title || '')}`);
          if (findResponse.ok) {
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mb-4">
                {movie.year && <span className="text-lg text-gray-600">{movie.year}</span>}
                {movie.genre && <span className="text-sm bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full">{movie.genre}</span>}
                {movie.director && <span className="text-sm text-gray-600">Director: {movie.director}</span>}
              </div>
              {averageRating > 0 && (
                <div className="mb-4">
