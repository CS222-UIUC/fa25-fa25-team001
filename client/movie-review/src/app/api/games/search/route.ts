 ...
 * GET /api/games/search?q=query&limit=20
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Format games with cover images and filter out games without ratings
    const formattedGames = games
      .filter(game => game.rating != null) // Only include games with ratings
      .map(game => ({
        id: game.id,
        name: game.name,
        slug: game.slug,
